...
class BookStore:
    ...
    def initialize_sample_book_data(self):
        sample_books = [
            Book("Python Programming", "Gerard Nwazk", "080312345678", 29.99, 10),
            Book("Data Structures", "Caleb Onyemechi", "080456789012", 39.99, 5),
    ...
    def display_menu(self):
        while True:
            ...
            if not self.current_user:
                ...
                print(f"""
                    \n=== Welcome {self.current_user.get_first_name()} ===
                    1. Add Book
                    2. Browse Books
                    3. Search Books
    ...
    def register(self):
        try:
            ...
            self.current_user = User(first_name, last_name, email, phone, gender)
    ...
    def search_books(self):
        self.clear_screen()
        search_term = input("\nEnter search term: ")
        results = [book for book in self.books if search_term.lower() in book.get_title().lower()]
        print("\n=== Search Results ===")
    ...
    def view_cart(self):
        ...
        if not self.shopping_cart.get_items():
            ...
            for book, quantity in self.shopping_cart.get_items().items():
                print(f"{book.get_title()} x{quantity} - ${book.get_price() * quantity:.2f}")
    ...
    def add_to_cart(self):
        ...
        try:
            ...
            if 0 <= choice < len(self.books):
                self.shopping_cart.add_item(self.books[choice], quantity)
    ...
    def remove_from_cart(self):
        ...
        if not self.shopping_cart.get_items():
            ...
            for i, (book, quantity) in enumerate(cart_items, 1):
                print(f"{i}. {book.get_title()} x{quantity}")
            
            try:
                choice = int(input("\nEnter item number to remove: ")) - 1
                quantity = int(input("Enter quantity to remove: "))
                
                if 0 <= choice < len(cart_items):
                    book = cart_items[choice][0]
                    self.shopping_cart.remove_item(book, quantity)
    ...
    def view_order_history(self):
        self.clear_screen()
        user_orders = [order for order in Order.orders if order._Order__user == self.current_user]
        ...
        if not user_orders:
            ...
            for order in user_orders:
                ...
                for book, quantity in order.get_items().items():
                    print(f"  - {book.get_title()} x{quantity}")
        input("\nPress Enter to continue...")
